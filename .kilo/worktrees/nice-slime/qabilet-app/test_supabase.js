...
async function check() {
  ...
  let { data: c, error: ce } = await supabase.from('courses').select('*').limit(1);
  console.log("courses:", c, ce);
  ...
  let { data: ch, error: che } = await supabase.from('chat_history').select('*').limit(1);
  console.log("chat_history:", ch, che);
}
